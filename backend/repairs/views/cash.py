...
Money out = every purchase (device lots AND parts orders). Money in = exit
sale money net of fees; a returned kind's refund arrives via sale_price the
same way. Parts installed into devices are already purchase rows, so nothing
double-counts.
"""
...
from repairs.models import Exit, Purchase
...
class CashSummaryView(APIView):
    ...
    def get(self, request):
        money_out = Purchase.objects.aggregate(total=Sum("total_price"))["total"] or Decimal("0")
