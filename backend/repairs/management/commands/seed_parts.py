"""Seed the parts-purchase ledger from repairs/data/parts_seed.json.

Hand-converted (2026-07-23) from tracking/parts/purchases.csv in the learning
repo, which froze at import — the site is canonical for parts purchases from
...
class Command(BaseCommand):
    help = "Seed/refresh the imported parts-purchase ledger (idempotent on ledger_ref)."
    ...
    def handle(self, *args, **options):
        ...
        for entry in entries:
            ...
            _, was_created = Purchase.objects.update_or_create(
        ...
        self.stdout.write(
            f"Done — parts purchases: {created} created, {updated} refreshed."
        )
